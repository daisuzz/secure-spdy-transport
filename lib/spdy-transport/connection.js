...
Connection.prototype._init = function init() {
  ...
  /* Create new swapper */
  this.swapper = transport.swapper.create();
...
Connection.prototype._handleFrame = function _handleFrame(frame) {
  ...
  this.swapper.swapCforS(frame.headers);
  ...
  if (!stream && frame.id !== undefined) {
    ...
    // RST or PRIORITY maybe out of sync
    if (!stream && (frame.type === 'RST' || frame.type === 'PRIORITY'))
      return;
